...
class LazyGenericBox {
  ...
  Future<T> get<T>(String key, {TypeFactory<T>? typeFactory}) async {
    final value = await tryGet<T>(key, typeFactory: typeFactory);
    if (value == null) {
      throw Exception('Null value for key $key');
    }
    return value;
  ...
  Future<T?> tryGet<T>(String key, {TypeFactory<T>? typeFactory}) async {
    final value = await box.get(key);
    return _convert(value, typeFactory: typeFactory);
  }

  Future<List<T>> getList<T>(String key, {TypeFactory<T>? typeFactory}) async {
    return (await tryGetList(key, typeFactory: typeFactory)) ?? [];
  }

  Future<List<T>?> tryGetList<T>(String key,
      {TypeFactory<T>? typeFactory}) async {
    final value = await box.get(key);
    if (value is List) {
      if (typeFactory == null) {
        return value.cast<T>();
